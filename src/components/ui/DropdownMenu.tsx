...
export interface DropdownMenuProps extends React.HTMLAttributes<HTMLElement> {
   ...
   * component ([similarly to how it is implemented in `<Dropdown.Menu>`](https://github.com/react-bootstrap/react-bootstrap/blob/v0.31.5/src/DropdownMenu#L115-L119)).*
   */
...
const DropdownMenu: DynamicRefForwardingComponent<'div', DropdownMenuProps> = React.forwardRef<
  HTMLElement,
  DropdownMenuProps
>(
  (
    ...
    if (align) {
      if (typeof align === 'object') {
        ...
        if (keys.length) {
          ...
          const direction: AlignDirection = (align as Record<string, AlignDirection>)[brkPoint];
    ...
    if (popper?.placement) {
      ...
      (props as any)['x-placement'] = popper.placement;
    }
